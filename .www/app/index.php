...
if (!isset($_SESSION['username'])) {
    ...
    <style>
        body{display:flex;justify-content:center;align-items:center;height:100vh;background:#f5f5f5;font-family:system-ui;color:#222}
...
if ($action) {
    if ($action==='list') {
        ...
        echo json_encode(scanDirRecursive($www));
        exit;
    }

    if($action==='load'){
        $file = realpath($www.'/'.$_POST['file']);
        if(!$file || strpos($file,$www)!==0 || !is_file($file)) { http_response_code(400); echo 'Invalid file'; exit; }
    ...
    if($action==='move'){
        $file = realpath($www.'/'.$_POST['file']);
        $target = $www.'/'.trim($_POST['target']);
        if(!$file || strpos($file,$www)!==0) { http_response_code(400); echo 'Invalid file'; exit; }
        if(file_exists($target)) { http_response_code(400); echo 'Target exists'; exit; }
        rename($file,$target) or die('Move failed'); echo 'ok'; exit;
    }
...
.file-item.drop-target { background:#cce5ff; }
...
<h5>File Tree (drag files/folders)</h5>
...
<button class="btn btn-danger btn-sm mt-1" onclick="deleteFile()">Delete</button>
</div>
...
async function refreshFiles(){
  ...
  function render(items,parent){
    items.forEach(i=>{
      ...
      div.draggable = true;
      div.ondragstart = e => { e.dataTransfer.setData('text/plain', i.path); };
      div.ondragover = e => { e.preventDefault(); div.classList.add('drop-target'); };
      div.ondragleave = e => { div.classList.remove('drop-target'); };
      div.ondrop = async e => {
        e.preventDefault(); div.classList.remove('drop-target');
        const src = e.dataTransfer.getData('text/plain');
        const t = i.path+'/'+src.split('/').pop();
        await ajax('move',{file:src,target:t}); refreshFiles();
      };
...
async function deleteFile(){
  ...
  const r = await ajax('rm',{file:f});
  if(r==='ok') { alert('Deleted'); refreshFiles(); document.getElementById('editor').value=''; document.getElementById('preview').srcdoc=''; } 
  else alert(r);
}
