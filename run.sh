...
# sanity checks
if [ ! -d "$WWW" ]; then _err ".www not found at $WWW"; fi
...
# Export environment variables so PHP sees them (CONFIG_PATH and WEBROOT)
export WEBROOT="$WWW"
export CONFIG_PATH="$WWW/config/config.php"

# Create a lightweight router that sets DOCUMENT_ROOT and serves files.
ROUTER="$LOGS/.php_router_${PORT}.php"
cat > "$ROUTER" <<'PHP_ROUTER'
<?php
// Router generated by launcher. Do not edit.
$webroot = getenv('WEBROOT') ?: __DIR__ . '/../.www';
$docroot = realpath($webroot) ?: $webroot;

// Make PHP scripts see the intended document root and config path
...
# start PHP built-in server with router and explicit -t webroot
